...
settings_fsm(void)
{
	switch (SETTING_CURRENT_STATE)
	{
		...
		case VOLTAGE_PRECISION_SETTINGS:
			adjust_voltage_precision_settings();
			break;
		default:
...
{	
	switch(cursor)
	{
		...
		case 2:
			SETTING_CURRENT_STATE = LOAD_CURRENT_SETTINGS;
			adjust_load_current_settings(NONE);
			break;
		/* LCD line 3: Set voltage precision in decimal places */
		case 3:
			SETTING_CURRENT_STATE = VOLTAGE_PRECISION_SETTINGS;
...
void display_settings_menu(void)
{
	...
	sprintf(dsp_buff[3], "Battery Type: Li-Ion");	// in progress....
