...
void conduct_test(void)
{
	...
	clear_lcd();
	/* Display results... */
	sprintf(dsp_buff[0], "B1: %.3f  B1: %.3f",UNLOADED_battery_voltages[0] ,LOADED_battery_voltages[0]); //place voltages in display buffers for each line
	sprintf(dsp_buff[1], "B2: %.3f  B2: %.3f",UNLOADED_battery_voltages[1] ,LOADED_battery_voltages[1]);
	sprintf(dsp_buff[2], "B3: %.3f  B3: %.3f",UNLOADED_battery_voltages[2] ,LOADED_battery_voltages[2]);
	sprintf(dsp_buff[3], "B4: %.3f  B4: %.3f",UNLOADED_battery_voltages[3] ,LOADED_battery_voltages[3]);	
...
void ERROR_MESSAGE_1(PB_interrupt PB_press)
{
	...
	sprintf(dsp_buff[2], "press OK to");
	sprintf(dsp_buff[3], "Continue");
	update_lcd();
};
